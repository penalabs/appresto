	...
	<div class="wrapper">
		...
		<section class="content-wrapper">
			<div class="wrapper">
              ...
              $user_resto = $_SESSION['id'];
              $id = $_GET['id'];
              $querytotal = "SELECT pembayaran.* ,user_resto.* ,pemesanan.*
              FROM pembayaran
              JOIN user_resto ON user_resto.id = pembayaran.id_user_kasir
              JOIN pemesanan ON pemesanan.id = pembayaran.id_pemesanan
              WHERE pembayaran.id_pemesanan = '$id'";
              $hasiltotal = mysqli_query($koneksi,$querytotal);
              while($data=mysqli_fetch_array($hasiltotal)){
                $total_harga = $data['total_harga'];
                $uangbayar = $data['nominal'];
                $uangkembalian = $uangbayar-$total_harga;
                ?>
								<div class="form-group">
                  <label> Uang Bayar</label>
									<input type="text" class="form-control" value="<?php echo "Rp. ".number_format($data['nominal']).",-"; ?>" disabled>
								</div>
                <hr>
                <div class="form-group">
                  <label> Uang Kembalian</label>
									<input type="text" class="form-control" value="<?php echo "Rp. ".number_format($uangkembalian).",-"; ?>" disabled>
								</div>
              <?php }?>
							</div>
						</div>
