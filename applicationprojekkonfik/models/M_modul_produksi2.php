...
class M_modul_produksi extends CI_Model{
	...
	function tambah_data_permintaan_bahan($nama_permintaan){
		$hasil=$this->db->query("INSERT INTO permintaan_bahan (nama_permintaan,status,tanggal_permintaan,tanggal_pengiriman,tanggal_penerimaan)
      VALUES ('$nama_permintaan','draft','0000-00-00 00:00:00','0000-00-00 00:00:00','0000-00-00 00:00:00')");
	...
	function tampil_data_bahan_mentah($id_permintaan){
	...
	function tampil_data_pemesanan_menu(){
		...
		$hasil=$this->db->query("SELECT data1.* FROM(SELECT pemesanan_menu.`id` , pemesanan_menu.`id_menu` , menu.`menu` , SUM(`pemesanan_menu`.`jumlah_pesan`) AS jumlahPesan
		FROM `pemesanan_menu`
		...
		UNION
		SELECT data2.* FROM(SELECT pemesanan_menu.`id` , pemesanan_menu.`id_menu` , menu.`menu` , SUM(`pemesanan_menu`.`jumlah_pesan`) AS jumlahPesan
		FROM `pemesanan_menu`
		JOIN menu ON `menu`.`id` = `pemesanan_menu`.`id_menu`
