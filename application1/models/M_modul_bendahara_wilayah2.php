...
class M_modul_bendahara_wilayah extends CI_Model{
	...
	function tampil_data_pengeluaran_kanwil(){
		$hasil=$this->db->query("SELECT `pengeluaran_kanwil`.`id` AS id_pengeluaran_kanwil, `pengeluaran_kanwil`.*, `kanwil`.*, `operasional`.*
			FROM `pengeluaran_kanwil`
			JOIN kanwil ON kanwil.`id_kanwil` = `pengeluaran_kanwil`.`id_kanwil`
			JOIN `operasional` ON `operasional`.`id` = `pengeluaran_kanwil`.`id_operasional`");
		return $hasil;
	}
	function tambah_data_pengeluaran_kanwil($kanwil,$operasional,$nominal){
		$hasil=$this->db->query("INSERT INTO pengeluaran_kanwil (id_kanwil,id_operasional,nominal)
      VALUES ('$kanwil','$operasional','$nominal')");

		return $hasil;
	}
	function edit_data_pengeluaran_kanwil($id_pengeluaran_kanwil){
		$hasil=$this->db->query("SELECT `pengeluaran_kanwil`.*, `kanwil`.*, `operasional`.*
			FROM `pengeluaran_kanwil`
			JOIN kanwil ON kanwil.`id_kanwil` = `pengeluaran_kanwil`.`id_kanwil`
			JOIN `operasional` ON `operasional`.`id` = `pengeluaran_kanwil`.`id_operasional`
			WHERE `pengeluaran_kanwil`.`id` = '$id_pengeluaran_kanwil'");

		return $hasil;
	}
	function update_data_pengeluaran_kanwil($id_pengeluaran_kanwiledit,$kanwiledit,$operasionaledit,$nominaledit){
		$hasil=$this->db->query("UPDATE pengeluaran_kanwil
			SET pengeluaran_kanwil.`id_kanwil`  = '$kanwiledit',  `pengeluaran_kanwil`.`id_operasional` = '$operasionaledit', `pengeluaran_kanwil`.`nominal` = '$nominaledit'
			WHERE `pengeluaran_kanwil`.`id` = '$id_pengeluaran_kanwiledit' ");

		return $hasil;
	}
	function delete_data_pengeluaran_kanwil($id_pengeluaran_kanwil){
		$hasil=$this->db->query("DELETE FROM pengeluaran_kanwil WHERE id = '$id_pengeluaran_kanwil'");
